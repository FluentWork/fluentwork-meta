...
**变更说明**：首版。线上**不走** I15 设想的 `ai.tts.start` → Opus 解码器。走那条会静音。读草稿时以本篇的「生产路径」为准。
...
3. 若来自 `.aiSpeaking`：WSS `interrupt` 先于 `user.speech.start`
4. 网关停转发；火山继续生成
